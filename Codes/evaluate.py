...
class GroundTruthLoader(object):
    ...
    NAME_MAT_MAPPING = {
        ...
        TRAFFIC: os.path.join(DATA_DIR, 'traffic-train/traffic-train3.mat'),
    ...
    NAME_FRAMES_MAPPING = {
        ...
        TRAFFIC: os.path.join(DATA_DIR, 'traffic-train/testing/frames'),
    ...
    def __load_ucsd_avenue_subway_gt(self, dataset):
        ...
        for i in range(num_video):
            length = get_video_length(i) #so luong fame cua tung video test

            sub_video_gt = np.zeros((length,), dtype=np.int8)
            sub_abnormal_events = abnormal_events[i]
            if sub_abnormal_events.ndim == 1:
                sub_abnormal_events = sub_abnormal_events.reshape((sub_abnormal_events.shape[0], -1)) # dua ve ma tran cot
